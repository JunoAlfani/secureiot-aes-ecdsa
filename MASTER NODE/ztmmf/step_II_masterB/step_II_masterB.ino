...
void reconnectMQTT() {
  ...
  String clientId = "ESP32MasterB-" + String((uint32_t)(chipid >> 32), HEX) + String((uint32_t)chipid, HEX);
...
void loop() {
  ...
  if (client) {
    ...
    // ✅ Ubah dari format key-value ke JSON object
    StaticJsonDocument<128> plainDoc;
    plainDoc["id"] = id;
    plainDoc["temp"] = temp;
    plainDoc["hum"] = hum;
    plainDoc["batt"] = battery;
    plainDoc["ts"] = timestamp;

    String rawData;
    serializeJson(plainDoc, rawData);
